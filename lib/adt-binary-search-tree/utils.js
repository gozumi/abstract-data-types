...
------------------------------------------------------------------------------*/
exports.isValid =  function(tree) {

	if (tree.isEmpty()) {
		return true;
	}

	return _isValidLeftTree(tree.getLeftTree(), tree.getRootItem()) &&
			_isValidRightTree(tree.getRightTree(), tree.getRootItem());

};

function _isValidLeftTree(tree, parentKey) {

	if (tree.isEmpty()) {
		return true;
	}

	return parentKey > _getKey(tree.getRootItem()) &&
		_isValidLeftTree(tree.getLeftTree(), tree.getRootItem()) &&
		_isValidRightTree(tree.getRightTree(), tree.getRootItem());
}

function _isValidRightTree(tree, parentKey) {

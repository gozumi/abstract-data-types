...
var BinaryTree = function ADT_BinaryTree(arg) {
	...
	if (arg) {
		this._root = new TreeNode(arg, this.constructor);
	}
...
BinaryTree.prototype.getRightTree = function() {
	...
	if (this.isEmpty()) {
		throw new Error('adt-binary-tree.getRightTree(): Tried to get the right tree of an empty tree');
	} else {
...
/*---------------------------------------------------
	...
	This method returns the number of nodes in the
	tree
...
BinaryTree.prototype.count = function() {
	...
	return 1 + this.getLeftTree().count() + this.getRightTree().count();
